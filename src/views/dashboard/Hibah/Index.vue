<template>
  <div id="wpbody-content">
    <div class="wrap">
      ...
      <form class="search-form" style="float:right;margin-top:8px">
        <input type="search" v-model="search" placeholder="Cari hibah..." style="width:220px" />
      </form>
      ...
      <table v-else class="wp-list-table widefat striped">
        <thead>
          <tr>
            <th scope="col" class="sortable" @click="tsort('title')">Judul <span>{{ sl('title') }}</span></th>
            <th scope="col">Kategori</th>
            <th scope="col">Skema</th>
            <th scope="col" class="sortable" @click="tsort('date')">Tanggal <span>{{ sl('date') }}</span></th>
        ...
        <tbody>
          <tr v-for="row in filtered" :key="row.id">
            ...
            <td>{{ row._date }}</td>
            <td><span class="post-state" :class="row.status === 'publish' ? '' : 'draft'">{{ row.status === 'publish' ? 'Terbit' : 'Draft' }}</span></td>
          </tr>
          <tr v-if="!filtered.length">
            <td colspan="5" style="text-align:center;padding:60px 0;color:var(--wp-text-muted)">
              <p style="font-size:16px;font-weight:600">Belum ada hibah</p>
              <p>Klik <strong>Tambah Hibah</strong> untuk membuat event pertama.</p>
            </td>
          </tr>
        </tbody>
        <tfoot>
...
const pageNumbers = computed(() => { const totalPages = Math.ceil(total.value / perPage); const nums: number[] = []; for (let i = Math.max(1, page.value - 2); i <= Math.min(totalPages, page.value + 2); i++) nums.push(i); return nums })
