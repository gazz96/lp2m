<template>
  <div>
    <div class="wp-header"><h1>Kategori Hibah</h1></div>
    <p class="sub">Kategori untuk event hibah.</p>
    <div class="add-box">
      <input type="text" v-model="newName" placeholder="Nama kategori..." class="add-input" @keyup.enter="addKat" />
      <input type="text" v-model="newSlug" placeholder="slug" class="add-input slug" />
      <button class="btn btn-primary btn-sm" @click="addKat" :disabled="!newName.trim() || adding">Tambah</button>
    </div>
          ...
          <tr v-for="t in terms" :key="t.id">
            <td>
              <template v-if="editId===t.id">
                ...
                <button class="btn btn-sm btn-primary" @click="updateKat(t.id)">Simpan</button><button class="btn btn-sm" @click="editId=null">Batal</button>
              </template>
              <button v-else class="link-btn" @click="startEdit(t)">{{ t.name }}</button>
            </td>
            <td class="slug-cell">{{ t.slug }}</td><td>{{ t.count }}</td>
            <td><button v-if="editId!==t.id" class="btn btn-sm btn-del" @click="removeKat(t)">Hapus</button></td>
          </tr>
          <tr v-if="!terms.length"><td colspan="4" class="empty">Belum ada kategori.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
async function addKat() { const n=newName.value.trim();if(!n)return; adding.value=true;err.value='';const slug=newSlug.value.trim()||n.toLowerCase().replace(/[^a-z0-9]+/g,'-')
  try { const r=await window.fetch(`${SITE.apiBase}/kategori_hibah`,{method:'POST',headers:{'Content-Type':'application/json',...auth.authHeaders()},body:JSON.stringify({name:n,slug})})
    if(!r.ok) throw new Error((await r.json().catch(()=>({}))).message||'Gagal'); newName.value='';newSlug.value='';loadKat() } catch(e:any){err.value=e.message} finally{adding.value=false} }
function startEdit(t:typeof terms.value[0]) { editId.value=t.id;editName.value=t.name;editSlug.value=t.slug }
async function updateKat(id:number) { try { const r=await window.fetch(`${SITE.apiBase}/kategori_hibah/${id}`,{method:'POST',headers:{'Content-Type':'application/json',...auth.authHeaders()},body:JSON.stringify({name:editName.value,slug:editSlug.value})})
    if(!r.ok) throw new Error('Gagal'); editId.value=null;loadKat() } catch(e:any){err.value=e.message} }
async function removeKat(t:typeof terms.value[0]) { if(!confirm(`Hapus "${t.name}"?`))return
  try { const r=await window.fetch(`${SITE.apiBase}/kategori_hibah/${t.id}?force=true`,{method:'DELETE',headers:{...auth.authHeaders()}}); if(!r.ok)throw new Error('Gagal');loadKat() } catch(e:any){err.value=e.message} }
onMounted(loadKat)
</script>
<style scoped>
