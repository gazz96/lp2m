<template>
  <div class="wp-app" style="padding:32px;min-height:100vh">
    <div class="wp-page-header">
      <h1 class="wp-page-title">Kategori Hibah</h1>
    </div>
    <p class="wp-page-subtitle">Kelola kategori untuk mengelompokkan event hibah.</p>
      ...
      <div class="wp-card-body">
        <div style="display:flex;gap:12px;align-items:flex-end">
          <div class="wp-field" style="flex:1;margin-bottom:0">
            ...
            <input type="text" v-model="newName" placeholder="cth. Penelitian Dasar" @keyup.enter="addKat" />
          </div>
          <div class="wp-field" style="flex:1;margin-bottom:0">
            <label>Slug <span class="hint">(opsional)</span></label>
            <input type="text" v-model="newSlug" placeholder="penelitian-dasar" />
          </div>
          <button class="wp-btn wp-btn-primary" @click="addKat" :disabled="!newName.trim()||adding">{{ adding?'Menyimpan...':'Tambah Kategori' }}</button>
        </div>
            ...
            <td>
              <template v-if="editId===t.id">
                <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
                  ...
                  <button class="wp-btn wp-btn-primary wp-btn-sm" @click="updateKat(t.id)">Simpan</button>
              ...
              <template v-else>
                ...
                <button class="wp-btn-link danger" @click="removeKat(t)">Hapus</button>
              </template>
            </td>
    ...
    <div v-else class="wp-empty"><div class="wp-empty-title">Belum ada kategori</div><div class="wp-empty-sub">Gunakan form di atas untuk menambah.</div></div>
  </div>
</template>
...
async function addKat(){const n=newName.value.trim();if(!n)return;adding.value=true;err.value='';const s=newSlug.value.trim()||n.toLowerCase().replace(/[^a-z0-9]+/g,'-')
  try{const r=await window.fetch(`${SITE.apiBase}/kategori_hibah`,{method:'POST',headers:{'Content-Type':'application/json',...auth.authHeaders()},body:JSON.stringify({name:n,slug:s})})
...
async function updateKat(id:number){try{const r=await window.fetch(`${SITE.apiBase}/kategori_hibah/${id}`,{method:'POST',headers:{'Content-Type':'application/json',...auth.authHeaders()},body:JSON.stringify({name:editName.value,slug:editSlug.value})});if(!r.ok)throw new Error('Gagal');editId.value=null;loadKat()}catch(e:any){err.value=e.message}}
async function removeKat(t:typeof terms.value[0]){if(!confirm('Hapus "'+t.name+'"? Data event tidak akan terhapus.'))return;try{const r=await window.fetch(`${SITE.apiBase}/kategori_hibah/${t.id}?force=true`,{method:'DELETE',headers:{...auth.authHeaders()}});if(!r.ok)throw new Error('Gagal');loadKat()}catch(e:any){err.value=e.message}}
onMounted(loadKat)
</script>
