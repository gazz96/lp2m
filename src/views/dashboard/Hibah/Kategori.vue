<template>
  <div>
    <div class="page-head"><h1>Kategori Hibah</h1></div>
    <p class="page-sub">Kelola kategori untuk mengelompokkan event hibah.</p>
    ...
    <div class="form-card">
      <div class="fc-row">
        ...
        <div class="fc-field">
          <label>Slug <span class="opt">(opsional, auto dari nama)</span></label>
          <input type="text" v-model="newSlug" placeholder="penelitian-dasar" />
        </div>
        <button class="btn btn-primary" @click="addKat" :disabled="!newName.trim() || adding">
            ...
            <td>
              <template v-if="editId === t.id">
                <div class="edit-row">
                  ...
                  <button class="btn btn-sm btn-primary" @click="updateKat(t.id)">Simpan</button>
              ...
              <template v-else>
                ...
                <div class="row-acts">
                  ...
                  <span class="sep">|</span>
                  <button class="lnk del" @click="removeKat(t)">Hapus</button>
                </div>
              </template>
            </td>
    ...
    <div v-else-if="!adding" class="empty-state">Belum ada kategori. Gunakan form di atas untuk menambah.</div>
  </div>
</template>
...
async function loadKat() {
  try { const r = await window.fetch(`${SITE.apiBase}/kategori_hibah?per_page=100&orderby=name&order=asc`); if (r.ok) terms.value = await r.json() } catch {}
}
async function addKat() {
  ...
  try {
    const r = await window.fetch(`${SITE.apiBase}/kategori_hibah`, {
...
async function updateKat(id: number) {
  try {
    const r = await window.fetch(`${SITE.apiBase}/kategori_hibah/${id}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...auth.authHeaders() },
      body: JSON.stringify({ name: editName.value, slug: editSlug.value })
    })
    if (!r.ok) throw new Error('Gagal'); editId.value = null; loadKat()
  } catch (e: any) { err.value = e.message }
}
async function removeKat(t: typeof terms.value[0]) {
  if (!confirm(`Hapus "${t.name}"? Data event yang menggunakan kategori ini tidak akan terhapus.`)) return
  try { const r = await window.fetch(`${SITE.apiBase}/kategori_hibah/${t.id}?force=true`, { method: 'DELETE', headers: { ...auth.authHeaders() } }); if (!r.ok) throw new Error('Gagal'); loadKat() }
  catch (e: any) { err.value = e.message }
}
onMounted(loadKat)
</script>
