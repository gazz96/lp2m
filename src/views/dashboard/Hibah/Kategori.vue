    ...
    <div class="components-panel" style="margin-bottom:20px">
      <div class="components-panel__body">
        <div style="display:flex;gap:12px;align-items:flex-end">
          ...
          <WpButton variant="primary" @click="editingId ? saveKat() : addKat()" :disabled="!newName.trim()||adding">{{ adding?'Menyimpan...':(editingId?'Perbarui':'Tambah') }}</WpButton>
...
async function saveKat() {
  ...
  try {
    const r = await window.fetch(`${SITE.apiBase}/kategori_hibah/${editingId.value}`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...auth.authHeaders() }, body: JSON.stringify({ name: newName.value.trim(), slug: newSlug.value.trim() || undefined }) })
    if (!r.ok) throw new Error((await r.json().catch(() => ({}))).message || 'Gagal')
    cancelEdit(); loadKat()
  } catch (e: any) { err.value = e.message } finally { adding.value = false }
}
