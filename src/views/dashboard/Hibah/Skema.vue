<template>
  <div class="wrap">
    <h1>Skema Hibah</h1>
    <p style="color:var(--wp-text-secondary);margin-top:-8px;margin-bottom:20px">Kelola skema pendanaan untuk event hibah.</p>

    <div class="components-panel" style="margin-bottom:20px">
      <div class="components-panel__body">
        <div style="display:flex;gap:12px;align-items:flex-end">
          <div style="flex:1">
            <label class="components-base-control__label">Nama Skema</label>
          ...
          <WpButton variant="primary" @click="addSkm" :disabled="!newName.trim()||adding">{{ adding?'Menyimpan...':'Tambah' }}</WpButton>
        </div>
...
const columns: WpColumn[] = [
  {
    ...
    rowActions: (r) => [
      { label: 'Edit', className: 'edit', to: '/dashboard/hibah/skema' },
      { label: 'Hapus', className: 'trash', to: '/dashboard/hibah/skema' },
    ]
  },
  { key: 'slug', label: 'Slug' },
...
async function addSkm() {
  ...
  try {
    const r = await window.fetch(`${SITE.apiBase}/skema_hibah`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...auth.authHeaders() }, body: JSON.stringify({ name: n, slug }) })
