  ...
  <div>
    ...
    <div v-else class="card-list">
      <div v-for="item in items" :key="item.id" class="event-card">
        <div class="ec-header">
          <span class="badge" :class="(item as any).jenis_hibah">{{ labelJenis((item as any).jenis_hibah) }}</span>
        ...
        <p class="ec-skema">{{ item.skema || (item as any).kategori_hibah || '' }}</p>
...
.ec-header { display: flex; gap: 6px; margin-bottom: 10px; }
